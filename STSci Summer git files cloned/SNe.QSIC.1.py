...
ax.set_xlim(4400*u.AA, 5500*u.AA)
ax.grid(True)
wave = [4570, 4861, 5007, 5412]
for line in wave:
    plt.axvline(x = line)
plt.show()

#Identify the Region for flux extraction
region_list = [[4567,4573],[4858,4864],[5004,5010],[5409,5415]]

###input values to integrate EW through
input_list = []
input_list.append(input('Mg wavelengths:'))
input_list.append(input('Hb wavelengths:'))
input_list.append(input('OIII wavelengths:'))
input_list.append(input('HeII wavelengths:'))


ew_list = []
cont_lst = []
##iterate through each observation to calculate EW.
...
exp_data_ew = Table([date_list, ew1, ew2, ew3, ew4], names=['date', 'mg ew', 'Hb ew', 'OIII ew', 'HeII ew'])
ascii.write(exp_data_ew, sne_name +'.1.ew.txt', overwrite=True)

exp_data_cont = Table([date_list, cont1, cont2, cont3, cont4], names=['date', 'mg cont', 'Hb cont', 'OIII cont', 'HeII cont'])
ascii.write(exp_data_cont, sne_name +'.1.cont.txt', overwrite=True)
